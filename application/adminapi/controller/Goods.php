...
class Goods extends BaseApi
{
    ...
    public function index()
    {
        $parames = input();
        $where = [];
        if(!empty($parames['keyword'])){
            $keyword = $parames['keyword'];
            $where['goods_name'] = ['like', "%$keyword%"];
        }

        $list = \app\common\model\Goods::with('category,type,brand')
    ...
    public function save(Request $request)
    {
        ...
        try{
            ...
            $params['goods_attr'] = json($params['goods_attr']);
    ...
    public function read($id)
    {
        ...
        $type = \app\common\model\Type::with('specs,specs.spec_value,attrs')->find($info['type_id']);
        $info['type'] = $type;
        $this->ok($info);
    }
    ...
    public function edit($id)
    {
        ...
        //单独查询所属模型及规格属性等信息
        $goods['type'] = \app\common\model\Type::with('specs,specs.spec_value,attrs')->find($goods['type_id']);
